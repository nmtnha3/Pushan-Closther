...
<body class="overflow-x-hidden">
    ...
    <section class="w-full flex pt-[120px]">
        ...
        <main class="ml-[380px] grid justify-center items-center gap-[20px]">
            <h1 class="text-center font-semibold text-[22px]">Thêm sản phẩm</h1>
            @if (session('success'))
                <div class="alert alert-success text-blue-500">
                    {{ session('success') }}
                </div>
            @endif
            <form action="{{ route('addProduct') }}" method="POST" class="grid gap-[20px]" enctype="multipart/form-data">
                @csrf
                        ...
                        @foreach ($menus as $menu)
                            @if ($menu->parent_id != null)
                                ...
                                @if ($parentMenu && $parentMenu->parent_id != null)
                                    <option class="text-red-500" value="{{ $menu->id }}">
                                        {{ $menu->name }}</option>
                                @endif
                            @endif
                        @endforeach
                ...
                <div class="flex items-center">
                    ...
                    <div class="flex items-center gap-[50px]">
                        <img id="imageProduct-preview" src="" alt="" class="h-[150px]">
                ...
                <div class="flex items-center">
                    ...
                    <div class="w-[400px] h-[200px] max-h-[300px] overflow-y-auto scroll-container">
                        <textarea name="descriptionProduct" id="descriptionProduct" class="outline-none w-full h-full resize-none"></textarea>
                    </div>
                </div>
